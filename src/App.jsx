            ...
            <>
              ...
              {regrade && (
                <Panel title={`Re-graded the overridden items with v${regrade.version}`}>
                  ...
                  <div className="text-xs mt-3" style={{ color: C.muted }}>Agreement within 0.5: {regrade.rows.filter((r) => Math.abs(r.before - r.human) <= 0.5).length}/{regrade.rows.length} before, then {regrade.rows.filter((r) => r.after != null && Math.abs(r.after - r.human) <= 0.5).length}/{regrade.rows.length} after. Rubric v{regrade.version} is now the active version; grade a new batch to use it.</div>
